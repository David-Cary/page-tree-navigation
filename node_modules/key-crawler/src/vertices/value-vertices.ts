 ...
 */
export class PrimitiveVertex<T = unknown> implements ValueVertex<T> {
  readonly value: T
 ...
 */
export class ArrayVertex<T = unknown> implements KeyValueVertex<T[], number, T> {
  readonly value: T[]
 ...
 */
export class ObjectVertex<V = unknown> implements KeyValueVertex<Record<ValidKey, V>, ValidKey, V> {
  ...
  * createKeyIterator (): IterableIterator<ValidKey> {
  ...
  getKeyIndex (key: ValidKey): number | undefined {
    let count = 0
    for (const targetKey in this.value) {
      if (targetKey === key) {
        return count
      }
      count++
 ...
 */
export class DefinedObjectVertex<V = unknown> extends ObjectVertex<V> {
  ...
  * createKeyIterator (): IterableIterator<ValidKey> {
  ...
  getIndexedKey (index: number): ValidKey | undefined {
    const wrappedIndex = getWrappedIndex(index, this.keys.length)
